...
export function createPopcandyMcpServer(domain: PopcandyMcpDomain): McpServer {
  ...
  for (const [plural, kind] of [['components','component'],['patterns','pattern'],['templates','template'],['migrations','migration']] as const) {
    server.registerResource(
      `popcandy-${plural}`,
      new ResourceTemplate(`popcandy://${plural}/{id}`, {
        list: async () => ({ resources: domain.listResources().filter((resource) => resource.uri.startsWith(`popcandy://${plural}/`)).map((resource) => ({ uri: resource.uri, name: resource.name, description: resource.description, mimeType: resource.mimeType })) }),
      }),
