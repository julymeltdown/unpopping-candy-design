...
const packageManifests = [
  ...
  { name: '@unpopping-candy/theme', dependencies: { '@unpopping-candy/tokens': 'workspace:^' } },
  { name: '@unpopping-candy/icons', dependencies: { '@ant-design/icons': '6.3.2' } },
  {
  ...
  { name: '@unpopping-candy/registry', dependencies: { '@unpopping-candy/knowledge': 'workspace:^' } },
  {
...
function hasCode(error: unknown, code: CatalogCompatibilityError['code']): boolean {
...
test('catalog selection rejects a mix of known release versions', () => {
  ...
  assert.throws(select, (error) => hasCode(error, 'POPCANDY_VERSION_SET_MIXED'));
});
...
test('catalog selection rejects a package-name set absent from the manifest', () => {
  ...
  assert.throws(select, (error) => hasCode(error, 'POPCANDY_CATALOG_INCOMPATIBLE'));
});
...
test('catalog selection rejects ambiguous exact mappings', () => {
  ...
  // When: selection is attempted.
  const select = () => selectCatalogVersion(ambiguousManifest, installedVersions);
...
test('package index exposes the bundled compatibility selector contract', () => {
  ...
  const installedVersions = { '@unpopping-candy/knowledge': '0.2.0' };
