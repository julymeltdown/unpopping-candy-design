...
export function ModerationWorkspace() {
  const [selected, setSelected] = useState<(typeof queue)[number]["id"]>(
    queue[0].id,
  );
  ...
  return (
    <Container size="full" className="popcandy-moderation-workspace">
      <aside
        ...
        className="popcandy-moderation-workspace__queue"
      >
        <Stack gap={2}>
          {queue.map((item) => (
            <Button
      ...
      <main
        className="popcandy-moderation-workspace__evidence"
        aria-labelledby="case-title"
      >
        <Stack gap={4}>
          <header>
            <p>{current.risk} risk</p>
            <h1 id="case-title">{current.label}</h1>
          </header>
          <Tabs
          ...
          <Surface border padding="lg">
            <p>
              {tab === "evidence"
                ? "Present the reported content, surrounding context, and verified evidence here."
                : "Present prior decisions and appeal history here."}
            </p>
          </Surface>
        </Stack>
      </main>
      <aside
        ...
        className="popcandy-moderation-workspace__decision"
      >
        <Stack gap={4}>
          <Alert
            title="Decision requires a policy reference"
            description="Record scope, impact, notification, and appeal availability."
            tone="neutral"
          />
          <Inline>
            <Button variant="secondary">No violation</Button>
