...
test("registry exposes all catalog templates with deterministic checksums", async () => {
  ...
  assert.equal(manifest.packageVersion, bundledCatalog.packageVersion);
  assert.equal(manifest.templates.length, 5);
  assert.deepEqual(
    manifest.templates.map((entry) => entry.id),
    [...manifest.templates.map((entry) => entry.id)].sort(),
  );
...
test("dry-run produces a complete plan without mutating the target project", async () => {
  ...
  const plan = await service().scaffold({
    templateId: "template.vite-app-shell",
    projectRoot: root,
    mode: "dry-run",
  });
...
test("apply writes a template once and a second run is idempotent", async () => {
  ...
  const first = await service().scaffold({
    templateId: "template.vite-app-shell",
    projectRoot: root,
    mode: "apply",
  });
  ...
  assert.match(
    await readFile(join(root, "src/main.tsx"), "utf8"),
    /UnpoppingCandyProvider/,
  );

  const second = await service().scaffold({
    templateId: "template.vite-app-shell",
    projectRoot: root,
    mode: "apply",
  });
...
test("target paths cannot escape the project root through traversal, absolute paths, or symlinks", async () => {
  ...
  await assert.rejects(
    service().scaffold({
      templateId: "template.profile-settings",
      projectRoot: root,
      targetDirectory: "linked-outside",
      mode: "dry-run",
    }),
...
test("malicious registry metadata is rejected before reading a source file", async () => {
  ...
  const malicious: KnowledgeCatalog = {
    ...bundledCatalog,
    entries: bundledCatalog.entries.map((entry) =>
  ...
  await assert.rejects(
    service(malicious).manifest(),
    /unsafe|relative|registry source/i,
  );
});
...
test("template variables are strictly allow-listed and safely substituted", async () => {
  ...
  assert.match(
    await readFile(join(root, "src/profile-settings.tsx"), "utf8"),
