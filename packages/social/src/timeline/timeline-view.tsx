...
import { Button, EmptyState, Spinner } from '@unpopping-candy/ui';
...
export function TimelineView({ emptyDescription = 'Follow people or return later when new posts are available.', emptyTitle = 'Nothing here yet', error, footer, loading = false, loadingMore = false, onRetry, posts, renderPost, renderPostActions }: TimelineViewProps) {
  if (loading && posts.length === 0) return <div className="popcandy-timeline" aria-label="Loading timeline">{Array.from({ length: 5 }, (_, index) => <PostCardSkeleton key={index} />)}</div>;
  ...
  return (
    <div className="popcandy-timeline" data-popcandy-component="timeline" role="feed" aria-busy={loadingMore || undefined}>
      {posts.map((post) => renderPost ? <div key={post.id}>{renderPost(post)}</div> : <PostCard key={post.id} post={post} {...renderPostActions?.(post)} />)}
      {loadingMore ? <div className="popcandy-timeline__loading"><Spinner label="Loading more posts" /></div> : null}
      {footer}
    </div>
  );
}
